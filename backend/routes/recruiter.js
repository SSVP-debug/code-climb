  ...
  try {
    ...
    if (!domain || personalDomains.includes(domain)) {
      return res.status(400).json({
        error: "Please sign in with your company email (e.g. name@google.com), not a personal email.",
      });
    }
...
router.get("/candidates", requireRole("recruiter", "admin"), async (req, res) => {
  try {
    const {
      college,
      topic,
...
router.post("/skills-test", requireRole("recruiter", "admin"), async (req, res) => {
  try {
...
router.get("/skills-test/:id", requireRole("recruiter", "admin"), async (req, res) => {
  try {
