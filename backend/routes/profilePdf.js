...
const BASE_URL = process.env.FRONTEND_URL || "https://code-club-one.vercel.app";
...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const profileUrl   = `${BASE_URL}/u/${user.username || "anonymous"}`;
    ...
    doc.fontSize(12).fillColor("#71717a").font("Helvetica")
       .text(`@${user.username ?? "unknown"} · Level ${level} · ${BASE_URL.replace("https://","")}`, 50, 138);
    ...
    doc.text("Generated by Code Club · codeclub.in", doc.page.width/2, footerY, { align: "center", width: doc.page.width - 100 });
