    ...
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div
          ...
          className="absolute left-0 top-full mt-2 w-44 rounded-xl bg-[var(--surface)] border border-[var(--border)] shadow-lg py-1 z-50"
        >
          {WORKSPACES.map((ws) => (
            <button
              key={ws.id}
              ...
              className={`w-full text-left px-3 py-2 text-sm transition ${
                ws.id === current.id
                  ? "text-[var(--foreground)] font-medium bg-[var(--surface-elevated)]"
