...
function FeatureGrid() {
  return (
    <Reveal as="section" className="px-6 py-20 md:px-12 md:py-24">
      ...
      <ol className="mx-auto mt-12 max-w-2xl divide-y divide-ink-800">
        {FEATURES.map((f, i) => (
          <li key={f.title} className="flex items-start gap-5 py-6">
            <span className="mt-0.5 w-5 flex-shrink-0 font-mono-ui text-xs text-zinc-600">
            ...
            />
            <div>
              <h3 className={`font-display font-semibold text-white ${f.featured ? "text-lg" : ""}`}>
                {f.title}
                {f.tag && (
                  <span className="ml-2 font-mono-ui text-[11px] uppercase tracking-lp-label text-zinc-500">
                    · {f.tag}
                  </span>
                )}
              </h3>
