...
function ChapterDetail({ chapter, nextChapter, onBack }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Chapter hero */}
      <div className={`rounded-3xl border ${colors.border} bg-gradient-to-br ${colors.gradient} bg-[var(--surface)] p-6 sm:p-8`}>
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--muted-foreground)]">
              Chapter {chapter.chapterNumber}
            </p>
            <h2 className="mt-1 text-2xl font-black text-[var(--foreground)]">{chapter.title}</h2>
          </div>
        ...
        <p className="mt-3 text-sm text-[var(--muted-foreground)] max-w-xl leading-relaxed">{chapter.storyDescription}</p>

        <div className="mt-4 flex items-center gap-3 flex-wrap text-xs text-[var(--muted-foreground)]">
          {!hideDifficulty && (
            <span
